...
function getDateLowerLimit() {
    ...
    dateLowerlimit = todayDate.toISOString();
}
...
function loadTumbo() {
	//$('#gcal').empty();
    $('#gcal-address-check').empty().append('<i class="fa fa-spinner fa-spin"> </i>');
 ...
 //   $('#gcal').empty().append('<h3>loading ...</h3>');
    ...
    if (null == pageToken || pageToken == "undefined" || pageToken == "") {
        $('#gcal-address-check').empty().append('<i class="fa fa-exclamation-triangle"> </i>');
...
var maxNowSize = Math.floor(((( $(window).height() - 60 ) - $('#timeDateDiv').height() ) * .5) / sizeOfNowDiv );
 $('#nowDiv').bigtext({
        maxfontsize: maxNowSize
    });

var maxNextSize = Math.floor( ( ( ( $(window).height() - 60 ) - ( $('#timeDateDiv').height() + $('#nextDiv').height() ) )  * .7 ) / sizeOfNextDiv );
...
function myTimer() {
    ...
    $('#timeDiv').bigtext();
  $('#timeDiv2').bigtext({maxfontsize: ( $(window).height() - 60 ) * .6});
  //  $('#dateDiv2').bigtext({maxfontsize: ( $(window).height() - 60 ) * .25});
 //     $('#timeDiv2').bigtext();
    $('#dateDiv2').bigtext();
